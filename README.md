# Akshat Puri Games Portfolio

🔗 **Live Portfolio Site**: [akshatpuri.github.io/mywebsite/](https://akshatpuri.github.io/mywebsite/)

---

## 🎴 Portfolio Card Builder GUI

The project includes an interactive Python-based desktop application inside `tools/` that lets you construct, test, and safely delete project cards from `index.html` without manually editing code markup.

### Running the Builder

Ensure Python 3 is installed on your system, then launch the app:

```bash
py tools/add_project_card.py
```

*   Use **Add Project Card** to add a permanent entry.
*   Use **Add Dry-run Card** / **Remove Dry-run Card** for testing layouts visually.
*   Use the **Remove Existing Card** section at the bottom to select and permanently delete cards via the dropdown.